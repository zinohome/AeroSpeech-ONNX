...
func NewPool(cfg *config.TTSModelConfig, size int) (*Pool, error) {
	...
	for i := 0; i < size; i++ {
		...
		if err != nil {
			logger.Warnf("Failed to create TTS provider %d: %v", i, err)
			continue
		}

		// 预热Provider
		if err := provider.Warmup(); err != nil {
			logger.Warnf("Failed to warmup TTS provider %d: %v", i, err)
			provider.Release()
			continue
		}
	...
	logger.Infof("TTS pool initialized with %d providers", len(pool.providers))
